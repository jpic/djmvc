...
from djmvc_example.stage1.models import Document
...
def test_stage1_list_scoped_to_owner(client, admin_user):
    ...
    response = client.get(reverse("site:stage1:list"))
...
@pytest.mark.django_db
def test_stage1_detail_404_outside_scope(client, admin_user):
    owner = User.objects.create_user("owner", password="pass")
    other = User.objects.create_user("other", password="pass")
    grant_document_perm(owner, "view_document")
    ...
    response = client.get(reverse("site:stage1:detail", args=[doc.pk]))
    assert response.status_code == 404


@pytest.mark.django_db
def test_stage1_list_denied_without_view_permission(client, admin_user):
...
@pytest.mark.django_db
def test_stage1_list_allowed_with_view_permission(client, admin_user):
    reader = User.objects.create_user("reader", password="pass")
...
@pytest.mark.django_db
def test_stage1_bulk_delete_intersects_scope(client, admin_user):
    owner = User.objects.create_user("owner", password="pass")
    other = User.objects.create_user("other", password="pass")
    grant_document_perm(owner, "view_document")
    ...
    url = (
        reverse("site:stage1:deleteobjects")
    ...
    client.post(url, {"next": reverse("site:stage1:list")})
