...
def test_list_permission_superuser_only_by_default(rf, admin_user):
    user = djmvc.site.routes['stage0'].routes['list']
    request = rf.get('/stage0/')
    request.user = admin_user
    view = type(user)(request=request)
    assert view.permission_fullcode == 'stage0.list_stage0'
...
def test_crud_permissions_use_django_defaults(rf, admin_user):
    ...
    grant_perm(regular, 'stage0', 'change_stage0')

    stage0 = djmvc.site.routes['stage0']
    request = rf.get('/stage0/1/update/')
    request.user = regular
    item = Stage0.objects.create(name='A')
    view = type(stage0.routes['update'])(request=request, pk=item.pk)
    assert view.permission_fullcode == 'stage0.change_stage0'
    assert view.has_permission() is True
...
def test_dispatch_forbidden_for_authenticated_denied(rf, admin_user):
    ...
    regular = User.objects.create_user('regular', password='pass')
    list_route = djmvc.site.routes['stage0'].routes['list']
    request = rf.get('/stage0/')
    request.user = regular
    ...
    assert response.url == resolve_url('/auth/login/') + '?next=/stage0/'


@pytest.mark.django_db
def test_action_mixin_object_permission(rf, admin_user):
    item = Stage0.objects.create(name='mine')
    ...
    controller = djmvc.ModelController.clone(model=Stage0, routes=[RestrictedUpdate])()
    controller.build()
    request = rf.get('/')
    request.user = admin_user
...
def test_custom_backend_introspects_view(rf):
    ...
    class ViewIntrospectionBackend:
        ...
        def has_perm(self, user_obj, perm, obj=None):
            if obj is None:
                return False
            return obj.permission_shortcode == 'list'
...
def test_list_with_view_permission_shortcode(rf):
    ...
    reader = User.objects.create_user('reader', password='pass')
    grant_perm(reader, 'stage0', 'view_stage0')
    ...
    request.user = reader
    view = type(controller.routes['list'])(request=request)
    assert view.permission_fullcode == 'stage0.view_stage0'
    assert view.has_permission() is True